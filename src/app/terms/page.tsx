    ...
    <main style={{ maxWidth: 480, margin: '0 auto', background: '#fdf4f7', minHeight: '100vh', padding: '0 0 40px' }}>
      <div style={{ background: '#fff', borderBottom: '1px solid #FCE4EC', padding: '16px 20px', paddingTop: 'calc(env(safe-area-inset-top) + 16px)', position: 'sticky', top: 0, zIndex: 10 }}>
        <div style={{ display: 'flex', alignItems: 'center' }}>
          <BackButton title="利用規約" fallback="/" />
        </div>
      </div>
      ...
      <div style={{ padding: '24px 20px', fontSize: 13, color: '#333', lineHeight: 1.9 }}>
        ...
        <Section title="第1条（定義）">
          ...
          <Item n="4">「プレミアム機能」とは、月額¥980のプレミアムプランのみで利用可能な機能（プレミアム専用キャラ等）を指します。</Item>
        ...
        <Section title="第3条（料金・支払い）">
          ...
          <Item n="4">支払い方法：iOSアプリ版はApp Store経由、Androidアプリ版はGoogle Play経由、Webブラウザ版はクレジットカード・Apple Pay・Google Pay（Stripe経由）で処理されます。</Item>
          <Item n="5">iOS版はApple Inc.（App Store）、Android版はGoogle LLC（Google Play）、Web版はStripe Inc.の決済システムを通じて処理されます。</Item>
        </Section>

        <Section title="第4条（解約・退会・返金）">
          <Item n="1">サブスクリプションの解約はご利用環境によって異なります。iOSアプリ版はApp Store（設定→Apple ID→サブスクリプション）から、Androidアプリ版はGoogle Play（マイアプリ→サブスクリプション）から、Webブラウザ版はアプリの<a href="/app/settings" style={{ color: '#E91E63' }}>設定ページ</a>内「サブスク管理・解約」からいつでも解約可能です。解約後は次回更新日まで引き続きご利用いただけます。日割り返金は行いません。</Item>
