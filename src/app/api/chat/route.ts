...
【口調の維持・最重要ルール】
提案・説明をするときも、あなた自身のキャラクターの話し方・口調を絶対に維持すること。
- あなたのキャラクターとして自然な言い方で提案する
...
function isDeepDiveRequest(message: string): boolean {
  const t = message.trim()
  // 「①②③以外で」「④⑤⑥以外で」「⑦⑧⑨以外で」は別提案リクエストなので除外
  if (/[①②③④⑤⑥⑦⑧⑨⑩⑪⑫].{0,4}以外/.test(t)) return false
  return (
    // 番号だけ（①〜⑫、1〜9対応）
    /^[①②③④⑤⑥⑦⑧⑨⑩⑪⑫１２３４５６７８９456789123]$/.test(t) ||
    /^[①②③④⑤⑥⑦⑧⑨⑩⑪⑫１２３４５６７８９456789123][番つ目]/.test(t) ||
    // 「③について」「⑦を」系（①〜⑫対応）
    /[①②③④⑤⑥⑦⑧⑨⑩⑪⑫].{0,6}詳しく/.test(t) ||
    /[①②③④⑤⑥⑦⑧⑨⑩⑪⑫].{0,6}教えて/.test(t) ||
    /[①②③④⑤⑥⑦⑧⑨⑩⑪⑫].{0,6}知りたい/.test(t) ||
...
【必ずやること】
・感情をそのまま受け止める。キャラクターの口調で（あなたのキャラクターとして自然な言葉で）
...
【最重要ルール：即①②③提案】
- ユーザーが何らかのテーマや悩みに触れた場合は、即座に①②③を出す
- 「もっと詳しく教えて」「どんなことが気になってる？」などの質問は絶対NG
- 「ない」「むり」「そう」など、テーマが全く読み取れない単語1つの場合のみ、「どんなことを知りたい？」と一言だけ聞き直せる
- 例：「料理が上手くなりたい」→ 即①②③。「〇〇はどうしたらいい？」→ 即①②③。
...
【番号の絶対ルール】
・①〜⑫はそれぞれ固定の番号。提案が何セット出ていても変わらない
・「1」→①  「2」→②  「3」→③  「4」→④  「5」→⑤  「6」→⑥
・「7」→⑦  「8」→⑧  「9」→⑨  「10」→⑩  「11」→⑪  「12」→⑫
・「〇〇の中にない」などと言ってはいけない。会話履歴全体から該当番号を探す

手順1：会話履歴全体から、指定された番号の提案内容を見つける
手順2：その番号の提案「だけ」を説明する。他の番号の内容は絶対に出さない
...
- 選んでいない番号の内容を出さない
- ですます調禁止
- 「どれのこと？」「②は④⑤⑥にないけど」など聞き返し禁止
`

// ─── 相談モード：追加提案（④⑤⑥ / ⑦⑧⑨ / ⑩⑪⑫）───
const SOUDAN_ALTERNATIVE_PROMPT = `
【相談モード・追加提案】
ユーザーがこれまでの提案とは異なる視点での追加提案を求めています。

Step 1【橋渡し】「ちがう角度で考えてみると」など自然な1文
Step 2【追加提案】ユーザーのメッセージに含まれる番号（④⑤⑥ または ⑦⑧⑨ または ⑩⑪⑫）を使って3つ出す
  例：ユーザーが「⑦⑧⑨として」と言った場合は ⑦ ⑧ ⑨ を使う
  例：ユーザーが「④⑤⑥として」と言った場合は ④ ⑤ ⑥ を使う
  - これまでに出た全ての提案（①〜直前の番号）とは全く異なる方向性にする（言い換えは絶対NG）
Step 3【選択を促す】「気になった番号あった？詳しく話せるよ」で締める

【絶対ルール】
- 番号はユーザーが指定したものを使う（①②③は絶対に使わない）
