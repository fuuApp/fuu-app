...
Step 1【すぐ本題へ】選択を繰り返さず、すぐ説明に入る。
  NG：「①で行くんだね」「①を選んだんだね」（繰り返し禁止）
  OK：「それはね、〜」「具体的には〜」「そのやり方はね〜」など自然に入る

Step 2【1つに絞った具体的な説明】2〜3文のみ。
  - 「具体的にどうやるか」だけに絞る
  - 複数の話題・サブトピックをまとめて出さない
  - 1回の返答は1つのポイントだけ完結させる

Step 3【次の一歩】「まずは〇〇だけやってみて」1文で締める。

【ルール】
- 選ばれていない番号には絶対触れない
- 複数の問題を一度に解決しようとしない（お風呂の話をしているときはお風呂だけ）
- キャラクターの口調を維持。ですます調禁止
- 全体200文字以内を厳守
`
